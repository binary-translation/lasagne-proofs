...
open Eq using (_≡_; refl) renaming (sym to ≡-sym; trans to ≡-trans)
...
import Proof.Framework LabelLIMM dst (RAW-Restricted.wf dst-ok) as Ψ
...
... | yes refl = -- x ≡ e
  ...
  where
  rf[xe]⇒x≡p : {x : Event LabelLIMM} → rf src x src-elim-ev → x ≡ src-preserved-ev
  rf[xe]⇒x≡p (rf-dst (x , y , rf[xy] , refl , p))
    with ev[$⇒]eq elim∈ex (rfʳ∈ex dst-wf rf[xy]) p -- e ≡ y in target?
  ... | refl = ⊥-elim (disjoint-r/skip _ (×₂-applyʳ (rf-w×r dst-wf) rf[xy] , elim-ev-skip))
