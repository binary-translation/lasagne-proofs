...
open Eq using (_≡_; refl; cong) renaming (sym to ≡-sym)
...
src-co-init-first (co-newˡ dst-co[py] refl refl) y-init =
  ...
  in ⊥-elim (disjoint-init/rwₙₜ _ (pres-init , wₙₜ⇒rwₙₜ preserved-ev-wₙᵣ))
...
src-co-init-first (co-ep refl refl) pres-init =
  ⊥-elim (disjoint-init/rwₙₜ _ (Init[$⇒] preserved∈ex pres-init , wₙₜ⇒rwₙₜ preserved-ev-wₙᵣ))
...
src-co-trans (co-ep refl refl) (co-dst (_ , _ , co[py] , τ , refl))
  with ev[$⇒]eq preserved∈ex (coˡ∈ex dst-wf co[py]) τ
...
src-co-trans (co-ep refl refl) (co-newʳ co[pp] τ refl) =
  ⊥-elim (co-irreflexive dst-wf (≡-sym (ev[$⇒]eq preserved∈ex (coˡ∈ex dst-wf co[pp]) τ)) co[pp])
...
private
  ...
  pˢ-sloc-pᵗ : SameLoc src-preserved-ev preserved-ev
  pˢ-sloc-pᵗ = same-loc src-preserved-has-loc preserved-has-loc

  loc-eˢ⇒pᵗ : (loc : Location) → HasLoc loc src-elim-ev → HasLoc loc preserved-ev
  loc-eˢ⇒pᵗ _ = subst-sloc (trans-same-loc (sym-same-loc pe-sloc) pˢ-sloc-pᵗ)

  coˡ-p⇒e : {y : Event LabelLIMM} → co src src-preserved-ev y → co src src-elim-ev y
  ...
  ... | no ¬y-elim
    with co-tri dst-wf elim-loc
           (with-pred preserved-ev (preserved-ev-w , preserved-has-loc , preserved∈ex))
...
src-co-sloc = ⊆: lemma
  ...
  lemma e y (co-newˡ dst-co[py] refl refl) =
    ...
    in trans-same-loc (sym-same-loc pe-sloc) (sloc[⇐] preserved∈ex y∈dst py-sloc)
  lemma x e (co-newʳ dst-co[xp] refl refl) =
    let x∈dst = coˡ∈ex dst-wf dst-co[xp]
        xp-sloc = ⊆₂-apply (co-sloc dst-wf) dst-co[xp]
    in trans-same-loc (sloc[⇐] x∈dst preserved∈ex xp-sloc) pe-sloc
