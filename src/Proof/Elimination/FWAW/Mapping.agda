...
module Proof.Elimination.FWAW.Mapping
...
events-preserved : ∀ {xᵗ : Event LabelLIMM} → xᵗ ≢ elim-ev → xᵗ ∈ events dst → xᵗ ∈ events src
events-preserved {event-init _ _ _} ¬xᵗ-elim x∈dst = events[⇐] x∈dst
events-preserved xᵗ@{event-skip uid tid} ¬xᵗ-elim x∈dst with ℕ-dec uid elim-uid
... | yes refl    = ⊥-elim (¬xᵗ-elim (unique-ids dst-wf uid (has-uid-skip , x∈dst) (elim-has-uid , elim∈ex)))
