...
module Proof.Reorder.Reorder21.Consistent
...
src-ax-coherence {x} refl = ax-coherence dst-ok refl ∘ ⁺-map (λ τ → τ) coh[⇒]
  ...
  coh[⇒] {x} {y} (coh-po-loc (po[xy] , xy-sloc@(same-loc _ y-loc))) =
    let y≢1 = λ{refl → ¬f-loc ev₁-f (_ , y-loc)}
    in coh-po-loc (po[⇒]ʳ y≢1 po[xy] , xy-sloc)
...
src-ax-global-ord refl ghb[xx] = 
  let -- If there exists a GHB cycle in the source, then there exists a detour cycle
      ...
      dst-ghb-alt[zz] = ⁺-map (λ τ → τ) ghbi[⇒] src-ghb-alt[zz]
